...
def validate_scannet(
    ...
    for i, sample in enumerate(val_dataset):
        ...
        if inference_size is None:
            padder = InputPadder(
                img_ref.shape, padding_factor=padding_factor, mode="kitti"
            )
            img_ref, img_tgt = padder.pad(img_ref, img_tgt)
        else:
            ori_size = img_ref.shape[-2:]
            img_ref = F.interpolate(
                img_ref, size=inference_size, mode="bilinear", align_corners=True
            )
            img_tgt = F.interpolate(
                img_tgt, size=inference_size, mode="bilinear", align_corners=True
            )
        ...
        with torch.no_grad():
            pred_depth = model(
                ...
                pose=pose,
                min_depth=1.0 / max_depth,
                max_depth=1.0 / min_depth,
        ...
        if save_vis_depth:
            ...
            viz_inv_depth = viz_depth_tensor(
                1.0 / pred_depth.cpu(), return_numpy=True, colormap="plasma"
            )  # [H, W, 3] uint8
...
def inference_depth(
    ...
    imgs = sorted(
        glob(os.path.join(inference_dir, "color", "*.jpg"))
    ...
    for i in range(len(imgs) - 1):
        ...
        intrinsics = (
            np.loadtxt(intrinsics_file).astype(np.float32).reshape((4, 4))[:3, :3]
        )  # [3, 3]

        pose_ref = (
            np.loadtxt(poses[i], delimiter=" ").astype(np.float32).reshape((4, 4))
        )
        pose_tgt = (
            np.loadtxt(poses[i + 1], delimiter=" ").astype(np.float32).reshape((4, 4))
        )
        # relative pose
        ...
        nearest_size = [
            int(np.ceil(img_ref.size(-2) / padding_factor)) * padding_factor,
        ...
        viz_inv_depth = viz_depth_tensor(
            1.0 / pr_depth.cpu(), return_numpy=True
        )  # [H, W, 3] uint8
        Image.fromarray(viz_inv_depth).save(filename)
        ...
        if pred_bidir_depth:
            ...
            filename = os.path.join(
                output_path, os.path.basename(imgs[i])[:-4] + "_bwd.png"
            )
            viz_inv_depth = viz_depth_tensor(
                1.0 / pr_depth_bwd.cpu(), return_numpy=True
            )  # [H, W, 3] uint8
            Image.fromarray(viz_inv_depth).save(filename)
