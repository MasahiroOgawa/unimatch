    ...
    def __getitem__(self, index):
        ...
        if self.half_resolution:
            sample["left"] = cv2.resize(
                sample["left"], None, fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR
            )
            sample["right"] = cv2.resize(
                sample["right"], None, fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR
            )

            sample["disp"] = (
                cv2.resize(
                    sample["disp"], None, fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR
                )
                * 0.5
            )
...
class FlyingThings3D(StereoDataset):
    def __init__(
        ...
        left_files = sorted(
            glob(data_dir + "/" + split + "/" + mode + "/*/*/left/*.png")
        )
...
class VKITTI2(StereoDataset):
    def __init__(
        ...
        for left_name in left_files:
            ...
            sample["disp"] = (
                left_name.replace("/rgb/", "/depth/").replace("rgb_", "depth_")[:-3]
                + "png"
            )
...
class MiddleburyEval3(StereoDataset):
    def __init__(
        ...
        super(MiddleburyEval3, self).__init__(
            transform=transform, is_middlebury_eth3d=True
        )
        ...
        if mode == "training":
            disp_files = sorted(
                glob(data_dir + "/" + mode + resolution + "/*/disp0GT.pfm")
            )
            assert len(left_files) == len(right_files) == len(disp_files)
        else:
            assert len(left_files) == len(right_files)
...
class Middlebury20052006(StereoDataset):
    def __init__(
        ...
        super(Middlebury20052006, self).__init__(
            transform=transform, is_raw_disp_png=True
        )
        ...
        dirs = [
            curr_dir
            for curr_dir in sorted(os.listdir(data_dir))
            if not curr_dir.endswith(".zip")
        ]
        ...
        for curr_dir in dirs:
            ...
            for illum in ["Illum1", "Illum2", "Illum3"]:
                for exp in ["Exp0", "Exp1", "Exp2"]:
                    ...
                    sample["left"] = os.path.join(
                        data_dir, curr_dir, illum, exp, "view1.png"
                    )
                    sample["right"] = os.path.join(
                        data_dir, curr_dir, illum, exp, "view5.png"
                    )
                    sample["disp"] = gt_disp
...
class Middlebury2014(StereoDataset):
    def __init__(
        ...
        super(Middlebury2014, self).__init__(
            transform=transform,
            is_middlebury_eth3d=True,
        ...
        for curr_dir in dirs:
            for data_type in ["", "E", "L"]:
                ...
                sample["right"] = os.path.join(
                    data_dir, curr_dir, "im1" + "%s.png" % data_type
                )
...
class Middlebury2021(StereoDataset):
    def __init__(
        ...
        super(Middlebury2021, self).__init__(
            transform=transform, is_middlebury_eth3d=True
        )
...
class CARLA(StereoDataset):
    def __init__(
        ...
        super(CARLA, self).__init__(
            transform=transform, is_middlebury_eth3d=True, half_resolution=True
        )
...
class InStereo2K(StereoDataset):
    def __init__(
        ...
        left_files = sorted(
            glob(data_dir + "/train/*/*/left.png") + glob(data_dir + "/test/*/left.png")
        )
        right_files = sorted(
            glob(data_dir + "/train/*/*/right.png")
            + glob(data_dir + "/test/*/right.png")
        )
        disp_files = sorted(
            glob(data_dir + "/train/*/*/left_disp.png")
            + glob(data_dir + "/test/*/left_disp.png")
        )
...
def build_dataset(args):
    if args.stage == "sceneflow":
        ...
        train_transform_list = [
            transforms.RandomScale(max_scale=0.4, crop_width=args.img_width),
            transforms.RandomCrop(args.img_height, args.img_width),
            transforms.RandomColor(),
            transforms.RandomVerticalFlip(),
            transforms.ToTensor(),
        ...
        train_transform_list = [
            transforms.RandomScale(
                min_scale=0, max_scale=1.0, crop_width=args.img_width
            ),
            transforms.RandomCrop(args.img_height, args.img_width),
        ...
        # middlebury 2016: 210
        mb2006 = Middlebury20052006(
            data_dir="datasets/Middlebury/2006", transform=train_transform
        )

        # middlebury 2014: 138, use half resolution
        mb2014 = Middlebury2014(half_resolution=True, transform=train_transform)
        ...
        # middlebury 2014: 138, use half resolution
        mb2014 = Middlebury2014(half_resolution=False, transform=train_transform)

        mb2014_half = Middlebury2014(half_resolution=True, transform=train_transform)

        # middlebury 2021: 115
        mb2021 = Middlebury2021(transform=train_transform)

        # middlebury eval3: 15
        mbeval3 = MiddleburyEval3(
            transform=train_transform,
            resolution="F",
        )

        # middlebury eval3 half: 15
        mbeval3_half = MiddleburyEval3(
            transform=train_transform,
            resolution="H",
        )
        ...
        train_transform_list = [
            transforms.Resize(scale_x=1.2, scale_y=1.5, nearest_interp=False),
            transforms.RandomScale(
                crop_width=args.img_width,
                nearest_interp=True,
            ),
