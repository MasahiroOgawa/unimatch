...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.no_normalize:
            ...
            sample["left"] = torch.from_numpy(left) / 255.0
        right = np.transpose(sample["right"], (2, 0, 1))

        if self.no_normalize:
            sample["right"] = torch.from_numpy(right)
        else:
            sample["right"] = torch.from_numpy(right) / 255.0
    ...
    def __call__(self, sample):
        ...
        if self.img_height > ori_height or self.img_width > ori_width:
            ...
            sample["left"] = np.lib.pad(
                sample["left"], ((top_pad, 0), (0, right_pad), (0, 0)), mode="edge"
            )
            sample["right"] = np.lib.pad(
                sample["right"], ((top_pad, 0), (0, right_pad), (0, 0)), mode="edge"
            )

            if "disp" in sample.keys():
                sample["disp"] = np.lib.pad(
                    sample["disp"],
        ...
        self.offset_y = np.random.randint(
            start_height, ori_height - self.img_height + 1
        )
    ...
    def __call__(self, sample):
        if np.random.rand() < 0.5:
            ...
            sample["left"] = cv2.resize(
                sample["left"], None, fx=scale_x, fy=1.0, interpolation=cv2.INTER_LINEAR
            )
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        sample["left"] = cv2.resize(
            sample["left"], None, fx=scale_x, fy=scale_y, interpolation=cv2.INTER_LINEAR
        )
        sample["right"] = cv2.resize(
            sample["right"],
            None,
...
class RandomOcclusion(object):
    def __init__(self, p=0.5, occlusion_mask_zero=False):
        self.p = p
