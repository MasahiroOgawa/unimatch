...
class ScannetDataset(Dataset):
    def __init__(
        ...
        for i in range(len(pairs)):
            ...
            if (
                not os.path.isfile(img_ref)
                or not os.path.isfile(img_tgt)
                or not os.path.isfile(depth)
                or not os.path.isfile(pose_ref)
                or not os.path.isfile(pose_tgt)
            ):
                continue
    ...
    def __getitem__(self, i):
        ...
        pose_ref = (
            np.loadtxt(pose_ref, delimiter=" ").astype(np.float32).reshape((4, 4))
        )
        pose_tgt = (
            np.loadtxt(pose_tgt, delimiter=" ").astype(np.float32).reshape((4, 4))
        )
        ...
        intrinsics = (
            np.loadtxt(intrinsics).astype(np.float32).reshape((4, 4))[:3, :3]
        )  # [3, 3]
    ...
    def _read_depth(self, filename):
        depth = np.array(Image.open(filename)).astype(np.float32) / 1000.0
...
class DemonDataset(Dataset):
    def __init__(
        ...
        scenes = [
            ...
            if os.path.isdir(os.path.join(os.path.join(data_dir, scene_dir)))
            and mode in scene_dir
        ]
        ...
        for scene in scenes:
            intrinsics = (
                np.genfromtxt(os.path.join(scene, "cam.txt"))
                .astype(np.float32)
                .reshape((3, 3))
            )  # [3, 3]
            ...
            for i in range(len(imgs)):
                if i < demi_length:
                    ...
                    shifts = list(
                        range(i - demi_length, i + (sequence_length + 1) // 2)
                    )
                    shifts.pop(demi_length)

                img_ref = imgs[i]
                depth = os.path.join(
                    os.path.dirname(img_ref), os.path.basename(img_ref)[:-4] + ".npy"
                )
                pose_ref = np.concatenate(
                    (poses[i, :].reshape((3, 4)), np.array([[0, 0, 0, 1]])), axis=0
                )  # [4, 4]
                ...
                for j in shifts:
                    img_tgt = imgs[j]
                    pose_tgt = np.concatenate(
                        (poses[j, :].reshape((3, 4)), np.array([[0, 0, 0, 1]])), axis=0
                    )
                    pose = (pose_tgt @ np.linalg.inv(pose_ref)).astype(
                        np.float32
                    )  # [4, 4]
