...
def dump_example(dataset_name):
    ...
    file = h5py.File(
        os.path.join(path, "traindata", "{:}.h5".format(dataset_name)), "r"
    )
    ...
    for seq_idx, seq_name in enumerate(file):
        ...
        if (
            (dataset_name == "sun3d_train_1.6m_to_infm" and seq_idx == 7)
            or (dataset_name == "sun3d_train_0.4m_to_0.8m" and seq_idx == 15)
            or (
                dataset_name == "scenes11_train"
                and (
                    seq_idx == 2758
                    or seq_idx == 4691
                    or seq_idx == 7023
        ...
        dump_dir = os.path.join(
            path, "../train", dataset_name + "_" + "{:05d}".format(seq_idx)
        )
        ...
        for f_idx, f_name in enumerate(sequence):
            ...
            for dt_type in frame:
                ...
                if dt_type == "camera":
                    ...
                    pose = np.array(
                        [
                            [img[5], img[8], img[11], img[14] * scale],
                            [img[6], img[9], img[12], img[15] * scale],
                    ...
                    depth = np.array(
                        np.frombuffer(
                            decompress(img.tobytes(), dimension[0] * dimension[1] * 2),
                            dtype=np.float16,
                        )
                    ).astype(np.float32)
...
def preparedata():
    num_threads = 1
    SUB_DATASET_NAMES = [
        "rgbd_10_to_20_3d_train",
        "rgbd_10_to_20_handheld_train",
        "rgbd_10_to_20_simple_train",
        "rgbd_20_to_inf_3d_train",
        "rgbd_20_to_inf_handheld_train",
        "rgbd_20_to_inf_simple_train",
    ...
    if num_threads == 1:
        ...
        Parallel(n_jobs=num_threads)(
            delayed(dump_example)(scene) for scene in SUB_DATASET_NAMES
        )
