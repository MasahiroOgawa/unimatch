...
def get_args_parser():
    ...
    parser.add_argument(
        "--min_depth", default=0.5, type=float, help="min depth for plane-sweep stereo"
    )
    parser.add_argument(
        "--max_depth", default=10, type=float, help="max depth for plane-sweep stereo"
    )
...
def main(args):
    ...
    if args.eval:
        ...
        if "scannet" in args.val_dataset:
            ...
            results_str = "\t".join(
                "{}: {:.4f}".format(k, v) for k, v in results_dict.items()
            )
            print(results_str)

        if "demon" in args.val_dataset:
            results_dict = validate_demon(
                model_without_ddp,
    ...
    if args.dataset == "scannet":
        train_set = ScannetDataset(
            transforms=train_transform,
            mode="train",
        )

    elif args.dataset == "demon":
        train_set = DemonDataset(
            mode="train",
            transforms=train_transform,
        )
    else:
    ...
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            train_set, num_replicas=torch.cuda.device_count(), rank=args.local_rank
        )
    else:
        train_sampler = None
    ...
    while total_steps < args.num_steps:
        ...
        for i, sample in enumerate(train_loader):
            ...
            results_dict = model(
                ...
                pose=pose,
                min_depth=1.0 / args.max_depth,
                max_depth=1.0 / args.min_depth,
                num_depth_candidates=args.num_depth_candidates,
                task="depth",
            )
            ...
            if args.depth_grad_loss_weight > 0:
                depth_grad_loss = depth_grad_loss_func(
                    depth_preds, gt_depth, valid_mask, gamma=0.9
                )
            ...
            if args.local_rank == 0:
                logger.add_image_summary(img_ref[0], img_tgt[0], is_depth=True)
                ...
                if args.depth_loss_weight > 0:
                    ...
                    depth_vis = 1.0 / depth_preds[-1][0]

                    logger.add_depth_summary(depth_vis, 1.0 / gt_depth_vis)

            total_steps += 1

            if args.local_rank == 0:
                if (
                    total_steps % args.save_ckpt_freq == 0
