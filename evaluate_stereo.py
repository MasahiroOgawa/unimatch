...
from dataloader.stereo.datasets import (
    FlyingThings3D,
    KITTI15,
    ETH3DStereo,
    MiddleburyEval3,
...
def create_kitti_submission(
    ...
    for i, sample in enumerate(test_dataset):
        ...
        if inference_size is None:
            ...
            left = F.interpolate(
                left, size=inference_size, mode="bilinear", align_corners=True
            )
            right = F.interpolate(
                right, size=inference_size, mode="bilinear", align_corners=True
            )
        ...
        skimage.io.imsave(
            save_name, (pred_disp.cpu().numpy() * 256.0).astype(np.uint16)
        )


@torch.no_grad()
def create_eth3d_submission(
    model,
    output_path="output",
    padding_factor=16,
    attn_type=None,
    ...
    test_dataset = ETH3DStereo(
        mode=submission_mode, transform=val_transform, save_filename=True
    )
...
@torch.no_grad()
def create_middlebury_submission(
    model,
    output_path="output",
    padding_factor=16,
    attn_type=None,
...
def validate_things(
    ...
    print("Validation things EPE: %.3f, D1: %.4f" % (mean_epe, mean_d1))
...
def validate_kitti15(
    ...
    print(
        "Validation KITTI15 EPE: %.3f, D1: %.4f, 3px: %.4f"
...
def validate_eth3d(
    ...
    print("Validation ETH3D EPE: %.3f, 1px: %.4f" % (mean_epe, mean_thres1))
...
def validate_middlebury(
    ...
    print("Validation Middlebury EPE: %.3f, 2px: %.4f" % (mean_epe, mean_thres2))
...
def inference_stereo(
    ...
    if inference_dir is not None:
        ...
        left_filenames = sorted(
            glob(inference_dir_left + "/*.png") + glob(inference_dir_left + "/*.jpg")
        )
        right_filenames = sorted(
            glob(inference_dir_right + "/*.png") + glob(inference_dir_right + "/*.jpg")
        )
    ...
    for i in range(num_samples):
        ...
        save_name = os.path.join(
            output_path, os.path.basename(left_name)[:-4] + "_disp.png"
        )
        ...
        if pred_bidir_disp:
            ...
            save_name = os.path.join(
                output_path, os.path.basename(left_name)[:-4] + "_disp_right.png"
            )
